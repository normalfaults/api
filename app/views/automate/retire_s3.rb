...
# CME / Service / Provisioning / StateMachines / Methods / provisionRDS
...
$evm.log("info", "Retire S3: Enter Method.")
...
$evm.log("info", "Retire S3: Bucket name: #{bucket_name}")
begin
  ...
  $evm.log("error", "Retire S3: Invalid client token exception caught: #{e.message}.")
  #TODO: send_order_status("CRITICAL", order_id, "","#{e.message}")
  exit
rescue AWS::S3::Errors::InvalidParameterValue => e
  $evm.log("error", "Retire S3: Invalid parameter exception caught: #{e.message}")
  #TODO: send_order_status("CRITICAL", order_id, "","#{e.message}")
  exit
rescue AWS::S3::Errors => e
  $evm.log("error", "Retire S3: AWS S3 exception caught: #{e.message}")
  #TODO: send_order_status("CRITICAL", order_id, "","#{e.message}")
  exit
rescue Exception => e
  $evm.log("error", "Retire S3: General exception caught: #{e.message}")
  $evm.log("error", "Retire S3: General exception backtrace: #{e.message}")
  #TODO: send_order_status("CRITICAL", order_id, "","#{e.message}")
  exit
end

$evm.log("info", "Retire S3: Bucket deleted successfully.")
